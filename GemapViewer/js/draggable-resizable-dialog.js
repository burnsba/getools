...
var	_minW = 100, // The exact value get's calculated
	...
	*/
	...
	_onMouseDown = function(evt) {
		...
		if (!(evt.target === _dialog || evt.target === _dialogContent || evt.target === _dialogButtonPane || evt.target === _dialogTitle || evt.target === _buttons[0]))
			return;
		...
		let dialogStyle = getComputedStyle(_dialog);

		_startX = evt.pageX;
		_startY = evt.pageY;
		_startW = _dialog.clientWidth;
		_startH = _dialog.clientHeight;
		_startBorderW = parseInt(dialogStyle.getPropertyValue('border-width'));
		_leftPos = rect.left;
		_topPos = rect.top;
		if (_isButtonHovered) {
			...
			document.getElementById('mmap').style.pointerEvents = 'none';
		}
		else if (_resizeMode != '') {
			_isResize = true;
	...
	_onMouseMove = function (evt) {
		...
		if (!(evt.target === _dialog || evt.target === _dialogTitle || evt.target === _dialogContent || evt.target === _dialogButtonPane || evt.target === _buttons[0]) && !_isDrag && _resizeMode == '')
			return;
		if (_isDrag) {
			...
			if (dx < 0) {
				// Something with border is causing scrollbar to appear.
				// current border thickness: 1px.
				if (left + _startW + _startBorderW + 1 >= _maxX) {
					left = _maxX - _startW - _startBorderW - 2;
                }
			}
			if (dx > 0) {
				if (left < 0)
			...
			if (dy < 0) {
				// border thickness
				if (top + _startH + _startBorderW + 1 >= _maxY) {
					top = _maxY - _startH - _startBorderW - 2;
                }
			}
			if (dy > 0) {
				if (top < 0)
			...
			var cs, rm = '';
			if (evt.target === _dialog || evt.target === _dialogTitle || evt.target === _dialogContent || evt.target === _dialogButtonPane || evt.target === _buttons[0]) {
				var rect = _getOffset(_dialog);
	...
	_onMouseUp = function(evt) {
		...
		if (_isDrag) {
			...
			document.getElementById('mmap').style.pointerEvents = '';
		}
		else if (_isResize) {
			_setCursor('');
			_isResize = false;
	...
	_setDialogContent = function () {
		...
		*/
		...
		_dialogContent.style.height = _minH + 'px';
	},
	...
	_init = function(id, callback) {
		...
		_buttons = _dialog.querySelectorAll('input[type="button"].close-on-click');  // Ensure to get minimal width
